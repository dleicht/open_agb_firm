...
BEGIN_ASM_FUNC convert160pFrameFast
	@ Video suspend can return this worker to libn3ds core1Standby().
	...
	@ Private Core-1 stop wakeup, consumed manually while IRQs are masked.
	...
	convert160pFrameFast_frame_lp:
		...
		convert160pFrameFast_8l_lp:
			...
			convert160pFrameFast_wait_irq:
				...
				/* Check teardown before interpreting the wakeup as LGYCAP. */
	...
	convert160pFrameFast_stop:
		/*
		 * Capture may stop between scanline chunks. Flush all private dirty cache
		 * lines before graphics power-down; a complete final frame is not required.
		 */
		mov  r3, #0
		mcr p15, 0, r3, c7, c14, 0                 @ Clean + invalidate entire D-cache.
		mcr p15, 0, r3, c7, c10, 4                 @ Data Synchronization Barrier.

		/* Drain pending IRQs until the private stop IPI has been acknowledged. */
